    ...
    for (listSeq = 0;
        ...
        if (listSeq == 0 || !PPM_EQUAL(splineList.color, lastColor)) {
            ...
            fprintf(fileP, "<path style=\"%s:#%02x%02x%02x; %s:none;\" d=\"",
                    ...
                    (unsigned int)PPM_GETR(splineList.color),
                    (unsigned int)PPM_GETG(splineList.color),
                    (unsigned int)PPM_GETB(splineList.color),
...
output_svg_writer(FILE *                    const fileP,
    ...
    fprintf(fileP, "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"%d\" height=\"%d\">\n", width, height);
