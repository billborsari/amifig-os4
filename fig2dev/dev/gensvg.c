...
#ifdef GRADIENTS
static int
rgbGradientVal(int colorIndex)
{
	int gradient = -1;
	
	if (colorIndex >= NUM_STD_COLS)
	{
		gradient = 	user_colors[colorIndex - NUM_STD_COLS].grad;
	}
	return(gradient);
}

static float
rgbOpacityVal(int colorIndex)
{
	float Opacity = 1.0;
	
	if ((colorIndex >= NUM_STD_COLS) && (user_colors[colorIndex - NUM_STD_COLS].a > 0))
	{
		Opacity = (float)(user_colors[colorIndex - NUM_STD_COLS].a) / 255.0;
	}
	return(Opacity);
}
#endif
...
void gensvg_option (char opt, char *optarg)
{
    switch (opt) {
...
#ifdef GRADIENTS
	/* Write all gradient definitions here */
	fprintf (tfp, "<defs>\n");
	for (grad = 0; grad < MAX_GRD_COLS; grad++)
	{
		if (GradientTable[grad] != NULL)
		{
			...
			{
				fprintf (tfp, "<linearGradient id=\"grad%d\" gradientUnits=\"%s\"\n", grad, 
						(GradientTable[grad]->unit==1)?"userSpaceOnUse":"objectBoundingBox");
				fprintf (tfp, "\t x1=\"%d\%\" y1=\"%d\%\" x2=\"%d\%\" y2=\"%d\%\">\n", 
						GradientTable[grad]->x1, GradientTable[grad]->y1, GradientTable[grad]->x2, GradientTable[grad]->y2);
			}
			else
			{
				fprintf (tfp, "<radialGradient id=\"grad%d\" gradientUnits=\"%s\"\n", grad, 
						(GradientTable[grad]->unit==1)?"userSpaceOnUse":"objectBoundingBox");
				fprintf (tfp, "\t cx=\"%d\%\" cy=\"%d\%\" r=\"%d\%\" fx=\"%d\%\" fy=\"%d\%\">\n", 
						GradientTable[grad]->x1, GradientTable[grad]->y1, GradientTable[grad]->radius, GradientTable[grad]->x2, GradientTable[grad]->y2);
			}
			
			for (stop = 0; stop < GradientTable[grad]->stops; stop++)
			{
				fprintf (tfp, "<stop offset=\"\%d\%\" stop-color=\"#%6.6x\" stop-opacity=\"%4.3f\" />\n",
						GradientTable[grad]->Stop[stop].stopvalue, 
						(GradientTable[grad]->Stop[stop].color & 0x00FFFFFF),
						(float)((GradientTable[grad]->Stop[stop].color & 0xFF000000) >> 24)/255.0);
			}
			
			if (GradientTable[grad]->type == 0)
			{
				fprintf (tfp, "</linearGradient>\n");
			}
			else
			{
				fprintf (tfp, "</radialGradient>\n");
			}
...
double dx,dy,len,cosa=0,sina=0,cosa1=0,sina1=0;
double hl;
...
#ifdef GRADIENTS
	if (rgbGradientVal(l->fill_color) != -1)
	{
		/* Gradient definition */
		fprintf (tfp, "fill:url(#grad%d);\n", rgbGradientVal(l->fill_color));
	}
	else
#endif
    if (l->fill_style != -1 )
...
#else
	    fprintf (tfp, "fill:#%6.6x;fill-opacity:%4.3f;\n"
					, rgbFillVal (l->fill_color, (l->fill_style > 40 ? 20 : l->fill_style))
					, rgbOpacityVal(l->fill_color));
#endif
    fprintf (tfp, "\"/>\n");
	...
	// AKS: missing check. If line has forward arrow, cosa and sina have not been initialized
	if(l->for_arrow != NULL)
	{
	...
	// AKS: missing check. If line has no backward arrow, cosa1 and sina1 have not been initialized
...
#ifdef GRADIENTS
	if (rgbGradientVal(a->fill_color) != -1)
	{
		/* Gradient definition */
		fprintf (tfp, "fill:url(#grad%d)\n", rgbGradientVal(a->fill_color));
	}
	else
#endif
    if (a->fill_style != -1)
     ...
     F_ellipse *e;
{
...
#ifdef GRADIENTS
	if (rgbGradientVal(e->fill_color) != -1)
	{
		/* Gradient definition */
		fprintf (tfp, "fill:url(#grad%d);\n", rgbGradientVal(e->fill_color));
	}
	else
#endif
	if (e->fill_style != -1)
...
#else
	    fprintf (tfp, "fill:#%6.6x;fill-opacity:%4.3f;"
					, rgbFillVal (e->fill_color, (e->fill_style > 40 ? 20 : e->fill_style))
					, rgbOpacityVal(e->fill_color));
#endif
        if (e->style > 0) {
	...
	{
		/* Gradient definition */
		fprintf (tfp, "fill:url(#grad%d);", rgbGradientVal(e->fill_color));
	}
	else
#endif
	if (e->fill_style != -1)
