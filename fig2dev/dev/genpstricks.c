...
void genpstrx_option(char opt, char *optarg)
{
  int i, tmp_int;
  double tmp_dbl;
  char fn[256];
  ...
  case 'p':
    ...
    if (strcmp(optarg, "-") != 0) {
      ...
      for (i = 0; optarg[i] && i < sizeof Pic_convert_dir - 2; i++)
...
struct color_table_entry_t {
  ...
  {"black",     1, 0.00, 0.00, 0.00},	/* black */
  {"blue",      1, 0.00, 0.00, 1.00},	/* blue */
  {"green",     1, 0.00, 1.00, 0.00},	/* green */
  {"cyan",      1, 0.00, 1.00, 1.00},	/* cyan */
  {"red",       1, 1.00, 0.00, 0.00},	/* red */
  {"magenta",   1, 1.00, 0.00, 1.00},	/* magenta */
  {"yellow",    1, 1.00, 1.00, 0.00},	/* yellow */
  {"white",     1, 1.00, 1.00, 1.00},	/* white */
  {"bluei",     0, 0.00, 0.00, 0.56},	/* blue1 */
  {"blueii",    0, 0.00, 0.00, 0.69},	/* blue2 */
  {"blueiii",   0, 0.00, 0.00, 0.82},	/* blue3 */
  {"blueiv",    0, 0.53, 0.81, 1.00},	/* blue4 */
  {"greeni",    0, 0.00, 0.56, 0.00},	/* green1 */
  {"greenii",   0, 0.00, 0.69, 0.00},	/* green2 */
  {"greeniii",  0, 0.00, 0.82, 0.00},	/* green3 */
  {"cyani",     0, 0.00, 0.56, 0.56},	/* cyan1 */
  {"cyanii",    0, 0.00, 0.69, 0.69},	/* cyan2 */
  {"cyaniii",   0, 0.00, 0.82, 0.82},	/* cyan3 */
  {"redi",      0, 0.56, 0.00, 0.00},	/* red1 */
  {"redii",     0, 0.69, 0.00, 0.00},	/* red2 */
  {"rediii",    0, 0.82, 0.00, 0.00},	/* red3 */
  {"magentai",  0, 0.56, 0.00, 0.56},	/* magenta1 */
  {"magentaii", 0, 0.69, 0.00, 0.69},	/* magenta2 */
  {"magentaiii",0, 0.82, 0.00, 0.82},	/* magenta3 */
  {"browni",    0, 0.50, 0.19, 0.00},	/* brown1 */
  {"brownii",   0, 0.63, 0.25, 0.00},	/* brown2 */
  {"browniii",  0, 0.75, 0.38, 0.00},	/* brown3 */
  {"pinki",     0, 1.00, 0.50, 0.50},	/* pink1 */
  {"pinkii",    0, 1.00, 0.63, 0.63},	/* pink2 */
  {"pinkiii",   0, 1.00, 0.75, 0.75},	/* pink3 */
  {"pinkiv",    0, 1.00, 0.88, 0.88},	/* pink4 */
  {"gold",      0, 1.00, 0.84, 0.00}	/* gold */
};
     ...
     int type, style, pen_color;
{
  double dir_len,
    ...
    x_lft, y_lft, 
    x_rgt, y_rgt;
  char fillcolor_option[100];

  if (style == 0)
    strcpy(fillcolor_option, ",fillcolor=white");
  else if (pen_color == -1)
    fillcolor_option[0] = '\0';
  else
    sprintf(fillcolor_option, ",fillcolor=%s",
	    color_name_after_declare_color(pen_color));
  ...
  case TRIANGLE_ARROW:
    ...
    fprintf(tfp, 
	    "\\pspolygon[linewidth="DBL",fillstyle=solid%s]"
	    "("DBL","DBL")("DBL","DBL")("DBL","DBL")\n",
	    thickness, fillcolor_option, 
  ...
  case FILLED_DOT_ARROW:
    if (Verbose)
      fprintf(tfp, "%% filled dot arrow\n");
    /* these are never hollow */
    if (pen_color == -1)
      fillcolor_option[0] = '\0';
    else
      sprintf(fillcolor_option, ",fillcolor=%s",
	      color_name_after_declare_color(pen_color));
    fprintf(tfp,
	    "\\pscircle[fillstyle=solid%s]("DBL","DBL"){"DBL"}%%\n",
	    fillcolor_option,
	    x_hd, y_hd, len/2);
    break;
     ...
     double hatch_angle_offset;
{
  ...
  if ((flags & FO_FILL) && fill_style != -1) {
	...
	break;
      ...
      // negative sign on angle for fig'x strange clockwise system
     ...
     F_arc *arc;
{
  ...
  r = sqrt(sqr_dbl(x_0 - x) + sqr_dbl(y_0 - y));
...
void
genpstrx_grid(float major, float minor)
{
