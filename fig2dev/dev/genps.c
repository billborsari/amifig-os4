...
int	filtype;
#ifdef WIN32
int read_gif(char *filename, int filetype, F_pic *pic, int *llx, int *lly, FILE *tfp);
#else
extern	int	read_gif();
#endif
extern	int	read_pcx();
...
int genps_end(void)
{
	...
	getcwd(curdir, 1024);
	retval = chdir(ghostScriptPath);

	/* make the ghostscript command to generate the ASCII or TIFF file from the temp eps file */
	sprintf(gscom, 
 	    "%s -q  -dNOPAUSE -dBATCH -dSAFER -sDEVICE=%s -r72 -g%dx%d -sOutputFile=\"%s\" \"%s\"",
		   ghostScriptPath, asciipreview? "bit" : (tiffcolor? "tiff24nc": "tifflzw"),
