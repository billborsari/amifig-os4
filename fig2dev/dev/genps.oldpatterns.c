...
void putword(int word, FILE *file);
void append(char *infilename, FILE *outfile);
void appendBinary(char *infilename, FILE *outfile);
void appendhex(char *infilename, FILE *outfile, int width, int height);
...
static void fill_area(int fill, int pen_color, int fill_color, int ulx, int uly);
static void clip_arrows(F_line *obj, int objtype);
static void draw_arrow(F_line *obj, F_arrow *arrow, Point *points, int npoints, Point *fillpoints, int nfillpoints, int col);
...
static Boolean iso_text_exist(F_compound *ob);
static Boolean ellipse_exist(F_compound *ob);
static Boolean approx_spline_exist(F_compound *ob);
static void draw_gridline(float x1, float y1, float x2, float y2);
static void set_style(int s, double v);
static void reset_style(int s, double v);
...
struct	_rgb {
	...
	{0.00, 0.00, 0.56F},	/* blue1 */
	{0.00, 0.00, 0.69F},	/* blue2 */
	{0.00, 0.00, 0.82F},	/* blue3 */
	{0.53F, 0.81F, 1.00},	/* blue4 */
	{0.00, 0.56F, 0.00},	/* green1 */
	{0.00, 0.69F, 0.00},	/* green2 */
	{0.00, 0.82F, 0.00},	/* green3 */
	{0.00, 0.56F, 0.56F},	/* cyan1 */
	{0.00, 0.69F, 0.69F},	/* cyan2 */
	{0.00, 0.82F, 0.82F},	/* cyan3 */
	{0.56F, 0.00, 0.00},	/* red1 */
	{0.69F, 0.00, 0.00},	/* red2 */
	{0.82F, 0.00, 0.00},	/* red3 */
	{0.56F, 0.00, 0.56F},	/* magenta1 */
	{0.69F, 0.00, 0.69F},	/* magenta2 */
	{0.82F, 0.00, 0.82F},	/* magenta3 */
	{0.50F, 0.19F, 0.00},	/* brown1 */
	{0.63F, 0.25F, 0.00},	/* brown2 */
	{0.75F, 0.38F, 0.00},	/* brown3 */
	{1.00, 0.50F, 0.50F},	/* pink1 */
	{1.00, 0.63F, 0.63F},	/* pink2 */
	{1.00, 0.75F, 0.75F},	/* pink3 */
	{1.00, 0.88F, 0.88F},	/* pink4 */
	{1.00, 0.84F, 0.00}	/* gold */
    };
...
static	 struct hdr {
	...
	headers[]= {
			{"GIF", "GIF",		    3, read_gif_qimage,	False},
#ifdef V4_0
...
#endif
			{"EMF", "emffile", 4, read_emf,	True},
			{"BMP", "bmpfile", 4, read_bmp,	True},
			};

#define NUMHEADERS sizeof(headers)/sizeof(headers[0])

#define EMFINDEX (NUMHEADERS-2)
#define BMPINDEX (NUMHEADERS-1)

/******************************/
/* various methods start here */
/******************************/

void geneps_option(char opt, char *optarg)
{
	epsflag = True;
	pdfflag = False;
...
void genps_option(char opt, char *optarg)
{
	epsflag = False;
	pdfflag = False;
	gen_ps_eps_option(opt, optarg);
...
void genps_start(F_compound	*objects)
{
	char		 host[256];
	...
	int		 userllx=0, userlly=0, userurx=0, userury=0;
	struct paperdef	*pd;
	char		 psize[20];
	...
	fprintf(tfp, "%%%%Creator: %s Version %s Patchlevel %s %s\n", prog, VERSION, PATCHLEVEL, winfigVersion);
	...
	// AKS: I removed that magnifiation tweak. It's not necessary for WinFIG, because it uses
	// the same displasy scale in metric and imperial mode
#if defined(WINFIG)
...
void genps_grid(float major, float minor)
{
	float	lx, ly, ux, uy;
	float	x, y;
	double	thick, thin;
...
int genps_end(void)
{
	...
	sprintf(gscom,
 	    "gswin32c.exe -q  -dNOPAUSE -dBATCH -dSAFER -sDEVICE=%s -r72 -g%dx%d -sOutputFile=\"%s\" \"%s\"",
...
static void set_linejoin(int j)
{
	if (j != cur_joinstyle) {
	    cur_joinstyle = j;
	    fprintf(tfp, "%d slj\n", cur_joinstyle);
...
void genps_line(F_line *l)
{
	...
	if (l->type == T_ARC_BOX) {
	  ...
	  /* PICTURE OBJECT */
		...
		// EMF files cannot be recognized by just looking into the first few bytes
		if(!found)
		{
			char ext[256] = "";
			stcgfe(ext, l->pic->file);
			if(strcasecmp(ext, "emf") == 0)
			{
				found = True;
				i = EMFINDEX;
			}
		...
		if (found) {
			...
			/* and read it */
			if (((*headers[i].readfunc)(picf,filtype,l->pic,&pllx,&plly, tfp)) == 0) {
			...
			if (((*headers[i].readfunc)(realname,filtype,l->pic,&pllx,&plly, tfp)) == 0) {
			    fprintf(stderr,"%s: Bad %s format\n",l->pic->file, headers[i].type);
			    return;	/* problem, return */
			}
		    }
		...
		if ((l->pic->subtype == P_XPM || l->pic->subtype == P_PCX || l->pic->subtype == P_PNG || l->pic->subtype == P_GIF) && !psencode_header_done)
			    PSencode_header();
		...
		} else if (l->pic->subtype == P_GIF || l->pic->subtype == P_PCX || l->pic->subtype == P_JPEG || l->pic->subtype == P_PNG || l->pic->subtype == P_EMF || l->pic->subtype == P_BMP) {
		...
		/* EPS file */
		    ...
		    fflush(tfp);
		    while ((i = _read(_fileno(picf),buf,sizeof(buf))) > 0) {
			_write(_fileno(tfp),buf,i);
		    }
		    close_picfile(picf,filtype);
		}
	  ...
	  /* POLYLINE */
		...
		if (l->for_arrow || l->back_arrow) {
		    fprintf(tfp, "gs ");
		    clip_arrows((F_line *)l, O_POLYLINE);
		}
	...
	if (l->type != T_PIC_BOX) {	/* make sure it isn't a picture object */
		...
		if (l->back_arrow && l->thickness > 0)
		    draw_arrow((F_line *)l, l->back_arrow, bpoints, nbpoints, bfillpoints, nbfillpoints, l->pen_color);
		if (l->for_arrow && l->thickness > 0)
		    draw_arrow((F_line *)l, l->for_arrow, fpoints, nfpoints, ffillpoints, nffillpoints, l->pen_color);
	}
	if (multi_page)
	   fprintf(tfp, "} bind def\n");
}

void genps_spline(F_spline *s)
{
        do_split(s->depth);
...
static void genps_itp_spline(F_spline *s)
{
	...
	if (s->back_arrow && s->thickness > 0)
	    draw_arrow((F_line *)s, s->back_arrow, bpoints, nbpoints, bfillpoints, nbfillpoints, s->pen_color);

	if (s->for_arrow && s->thickness > 0)
	    draw_arrow((F_line *)s, s->for_arrow, fpoints, nfpoints, ffillpoints, nffillpoints, s->pen_color);
}
...
void genps_arc(F_arc	*a)
{
	...
	if (a->type == T_OPEN_ARC) {
		if (a->back_arrow && a->thickness > 0)
			draw_arrow((F_line *)a, a->back_arrow, bpoints, nbpoints, bfillpoints, nbfillpoints, a->pen_color);
		if (a->for_arrow && a->thickness > 0)
			draw_arrow((F_line *)a, a->for_arrow, fpoints, nfpoints, ffillpoints, nffillpoints, a->pen_color);
	}
	if (multi_page)
	   fprintf(tfp, "} bind def\n");
}

void genps_ellipse(F_ellipse	*e)
{
        do_split(e->depth);
...
void convert_xpm_colors(unsigned char cmap[3][MAXCOLORMAPSIZE], XpmColor *coltabl, int ncols)
{
	int	i;
	char	*name;
	RGB	rgb;
