...
void points_bound(F_point *points, int *xmin, int *ymin, int *xmax, int *ymax);
void int_spline_bound(F_spline *s, int *xmin, int *ymin, int *xmax, int *ymax);
void normal_spline_bound(F_spline *s, int *xmin, int *ymin, int *xmax, int *ymax);
...
double compute_angle(double dx, double dy);
// aks
...
double rot_x(double x, double y, double angle)
{
    return(x*cos(-angle)-y*sin(-angle));
}

double rot_y(double x, double y, double angle)
{
...
void control_points_bound(F_control *cps, int *xmin, int *ymin, int *xmax, int *ymax)
{
	F_control	*c;
	double		bx, by, sx, sy;
...
void calc_arrow(int x1, int y1, int x2, int y2, int linethick, F_arrow *arrow,
			Point points[], int *npoints, Point  fillpoints[], int *nfillpoints, Point  clippts[], int *nclippts)
{
...
double compute_angle(double dx, double dy)		/* compute the angle between 0 to 2PI  */
{
    double	    alpha;
...
void compute_arcarrow_angle(double x1, double y1, double x2, double y2, int direction, F_arrow *arrow, int *x, int *y)
{
    ...
    if (h > 2.0*r) {
	arc_tangent2(x1,y1,x2,y2,direction,x,y);
	return;
    }
