...
extern F_arrow *make_arrow(int type, int style, double thickness, double wid, double ht);
extern int read_1_3_objects(FILE *fp, F_compound *obj);
extern int read_1_4_objects(FILE *fp, F_compound *obj);
extern F_line *create_line_with_spline(F_spline *s);
extern int make_control_factors(F_spline *spl);
...
void print_comments(char	*string1, F_comment	*comment, char	*string2);
...
User_color	 user_colors[MAX_USR_COLS + MAX_GRD_COLS];
int		 user_col_indx[MAX_USR_COLS + MAX_GRD_COLS];
int		 num_usr_cols;
int		 gif_colnum = 0;
char		 buf[BUF_SIZE];
int		 line_no = 0;
...
static void read_colordef(FILE *fp)
{
    ...
    */
    ...
    if (    (sscanf(buf, "%*d %d", &c) != 1)
       ...
       ||   (c >= NUM_STD_COLS + MAX_USR_COLS + MAX_GRD_COLS) )                 /* colors above 1056 aren't allowed */
    
    {	
		put_msg("Invalid color definition 1 : %s.",buf);
    }
    else
    {
        ...
        if (c < MAX_USR_COLS)
        {
            ...
            if (read_result == 1)
            {
				user_col_indx[num_usr_cols] = c + NUM_STD_COLS;
				user_colors[num_usr_cols].r = (RGB & 0x00FF0000) >> 16;
				user_colors[num_usr_cols].g = (RGB & 0x0000FF00) >> 8;
				user_colors[num_usr_cols].b = (RGB & 0x000000FF);
				user_colors[num_usr_cols].a = (RGB & 0xFF000000) >> 24;
				user_colors[num_usr_cols].grad = -1;
            }
            else
            {
                /* Syntax error in color definition : set to default(black) and warn user */
				user_col_indx[num_usr_cols] = c + NUM_STD_COLS;
				user_colors[num_usr_cols].r = 0;
				user_colors[num_usr_cols].g = 0;
				user_colors[num_usr_cols].b = 0;
				user_colors[num_usr_cols].a = 0;
				user_colors[num_usr_cols].grad = -1;
                put_msg("Invalid color definition 2 : %s, setting to black (#00000).",buf);
            }
			return;
        }
        else
        {
            ...
            new_gradient = (Gradient *)malloc(sizeof(Gradient));
            if (new_gradient != NULL)
            {
				...
				if (read_result == 9)
				{
					...
					*/
					...
					if (GradientTable[c]) free(GradientTable[c]);
					GradientTable[c] = new_gradient;
					/* Set usr_col_indx and user_colors table to default values */
					user_col_indx[num_usr_cols] = c + NUM_STD_COLS + MAX_USR_COLS;
					user_colors[num_usr_cols].r = (GradientTable[c]->Stop[0].color & 0x00FF0000) >> 16;
					user_colors[num_usr_cols].g = (GradientTable[c]->Stop[0].color & 0x0000FF00) >> 8;
					user_colors[num_usr_cols].b = (GradientTable[c]->Stop[0].color & 0x000000FF);
					user_colors[num_usr_cols].a = (GradientTable[c]->Stop[0].color & 0xFF000000) >> 24;
					user_colors[num_usr_cols].grad = c;
					return;
				}
				/* error reading gradient after new_gradient creation */
				free(new_gradient);
            }
            put_msg("error allocating memory for gradient");
        }
		/* Syntax error in color definition : warn user */
		put_msg("Invalid color definition 3 : %s.",buf);
    }
#endif
}

void fix_color(int *color)
{
    int		    i;
