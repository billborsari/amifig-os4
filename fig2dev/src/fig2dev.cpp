...
#ifdef WIN32
// required for GetEnvironmentVariable()
#include <windows.h>
#endif
...
extern void compound_bound(F_compound *compound, int *xmin, int *ymin, int *xmax, int *ymax, int include);

char winfigVersion[]="WinFIG version 1 (11/29/2009) by Andreas Schmidt";

#ifdef WIN32
char TMPDIR[1024] = "";
#endif

int fig_getopt(int nargc, char **nargv, char *ostr);
extern	char	*optarg;
extern	int	 optind;
int parse_gridspec(char *string, float *numer, float *denom, float *spacing, int *nchrs);

void	help_msg();
void	depth_option(char *s);
...
char	gscom[2048];		/* to build up a command for ghostscript */
...
void grid_usage(void);
int gendev_objects(F_compound *objects, struct driver *device);
...
void free_gradients(void)
{
	...
	for (i=0; i<MAX_GRD_COLS; i++)
		if (GradientTable[i]) free(GradientTable[i]);
}
#endif
...
void free_objectsincompound(F_compound *object)
{
	free_arc(&object->arcs);
	free_compound(&object->compounds);
	free_ellipse(&object->ellipses);
	free_line(&object->lines);
	free_spline(&object->splines);
	free_text(&object->texts);
}
...
/*
void put_msg(char *format, char *arg1, char *arg2, char *arg3, char *arg4, char *arg5, char *arg6, char *arg7, char *arg8)
{
...
int main(int argc, char *argv[])
{
	...
	QString path = argv[0];
	int index = path.lastIndexOf(QDir::separator().toLatin1());

	QString homeDir;

	if(index != -1)
		homeDir = path.left(index);
	else
		homeDir = ".";

	app.addLibraryPath(homeDir + QDir::separator() + "plugins");
#endif

#ifdef WIN32
	if(GetEnvironmentVariable("TEMP", TMPDIR, 1024) == 0)
	{
		if(GetEnvironmentVariable("TMP", TMPDIR, 1024) == 0)
		{
			strcpy(TMPDIR, ".");
		}
	}
...
//	printf("%s\n", ghostScriptPath);
	...
	compound_bound(&objects, &llx, &lly, &urx, &ury, device->text_include);
	...
	/* If metric, adjust scale for difference between FIG PIX/CM (450) and actual (472.44)
	   ...
	   not inches or cm */
// AKS, WinFIG
// magnification must not be changed, because WinFIG calculates coordinates
// correctly for 472.44 PIX/CM (1200 dpi)
//	if (metric)
//		mag *= 80.0/76.2;

	status = gendev_objects(&objects, device);
	if ((tfp != stdout) && (tfp != 0)) 
	    (void)fclose(tfp);

	/* ! shouldn't we free the objects? */
	free_objectsincompound(&objects);
...
static int compound_dump(F_compound *com, struct obj_rec *array, int count, struct driver *device)
{
  	F_arc		*a;
	F_compound	*c;
	F_ellipse	*e;
	...
	for (c = com->compounds; c != NULL; c = c->next)
	  count = compound_dump(c, array, count, device);
	for (a = com->arcs; a != NULL; a = a->next) {
	  if (array) {
		array[count].gendev = (void (*)(void *))device->arc;
	...
	for (e = com->ellipses; e != NULL; e = e->next) {
	  if (array) {
		array[count].gendev = (void (*)(void *))device->ellipse;
	...
	for (l = com->lines; l != NULL; l = l->next) {
	  if (array) {
		array[count].gendev = (void (*)(void *))device->line;
	...
	for (s = com->splines; s != NULL; s = s->next) {
	  if (array) {
		array[count].gendev = (void (*)(void *))device->spline;
	...
	for (t = com->texts; t != NULL; t = t->next) {
	  if (array) {
		array[count].gendev = (void (*)(void *))device->text;
...
int mergesort(void *vbase, size_t nmemb, size_t size, int (*compar)(const void *, const void *));
...
int gendev_objects(F_compound *objects, struct driver *device)
{
	...
	obj_count = compound_dump(objects, 0, 0, device);
	if (!obj_count) {
	    fprintf(stderr, "fig2dev: No objects in Fig file\n");
	    return -1;
	}
	rec_array = (struct obj_rec *)malloc(obj_count*sizeof(struct obj_rec));
	(void)compound_dump(objects, rec_array, 0, device);

	/* sort object array by depth */
	// qsort(rec_array, obj_count, sizeof(struct obj_rec), (int (*)(const void *,const void *))rec_comp);
	// using heapsort from BSD sources instead isn't helping!
	// heapsort(rec_array, obj_count, sizeof(struct obj_rec), (int (*)(const void *,const void *))rec_comp);
	// try using mergesort implementation (YES ITS WORKING FINALY!);
	mergesort(rec_array, obj_count, sizeof(struct obj_rec), (int (*)(const void *,const void *))rec_comp);
	
	/* generate header */
	(*device->start)(objects);

	/* draw any grid specified */
	(*device->grid)(grid_major_spacing, grid_minor_spacing);
...
void gendev_null2(float p1, float p2) {
    ;
}
