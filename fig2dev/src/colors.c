...
int	numXcolors = sizeof(Xcolors)/sizeof(struct color_db);

#endif

int lookup_X_color(char *name, RGB  *rgb)
{
    int		i, n=0;
    int		r, g, b;
    struct color_db *col;
...
int read_colordb(void)
{
/* uncommented because colors are statically defined in the array initialization
    FILE	*fp;
...
*/
	...
	for(i=0; i < numXcolors; i++)
	{
		Xcolors[i].red		= Xcolors[i].red<<8;
		Xcolors[i].green	= Xcolors[i].green<<8;
		Xcolors[i].blue		= Xcolors[i].blue<<8;
	}
